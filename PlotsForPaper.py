...
"""
from DiffEqs import dark_matter_heating_per_second
for mX in [1e-2,1e-3,1e-4]:
    mphi = 1e-8*mX
    g_N = 1e-12
    zvec = np.logspace(1.,3.,101.)
    
    #plt.loglog(zvec,[dark_matter_heating_per_second(z,mX,mphi,g_N,1.) for z in zvec])
    #plt.xlim([1e1,1e3])
    #plt.xlabel('$z$')
    #plt.show()
    
    print(np.sum([dark_matter_heating_per_second(z,mX,mphi,g_N,1.)/(.3**.5*2e-18)*(1.+z)**(-2.5) for z in np.linspace(17.,1100.,1088)]))
    plt.semilogy(zvec,np.array([dark_matter_heating_per_second(z,mX,mphi,g_N,1.)/(.3**.5*2e-18)*(1.+z)**(-2.5) for z in zvec]))
    plt.xlim([1e1,1e3])
    plt.xlabel('$z$')
plt.show()
exit()
...
mX = 1e-3
mphi = 1e-9
T_tildes = T_tilde(17.,mX)*np.logspace(-1.,1.)
print(T_tildes)
plt.loglog(np.logspace(-1.,1.),[integration_over_MB(t,mX,mphi,'deexcitation') for t in T_tildes],'b')
#plt.loglog(T_tildes,[integration_over_MB(t,mX,mphi,'excitation') for t in T_tildes],'r')
plt.show()
exit()
#"""
...
#cs = plt.contour(np.log10(mphivec)+9.,np.log10(mXvec)+3.,rel_Ttilde_matrix,levels=[0.01,0.1,0.5,1.,2.,4.,5.,6.,7.,8.,9.,10.],colors='k',linestyles='-')
#plt.clabel(cs, inline=1, fontsize=10)
#clb = plt.colorbar()
#plt.contour(np.log10(mphivec)+9.,np.log10(mXvec)+3.,dTs_dz_matrix,levels=[0.],colors='r',linestyles=[':'])
plt.plot([],[],'k-',label=r'$\epsilon(\tilde{T})$')
#plt.plot([],[],'r:',label=r'd$T_s$/d$z=0$')
